...
def fuzzy_match_with_progress(file_a, file_b, output_file, gene_column_a, gene_id_column_b, collinear_gene_column_b, 
    ...
    模糊匹配结果以蓝色高亮显示。
    支持进度和状态回调。
    vertical: 若为True，结果竖向排列（每个基因一列，匹配结果向下扩展）。
    """
    if set_progress_status:
    ...
    if set_progress_status:
        set_progress_status("开始模糊与精确匹配")
    all_matches, all_flags = [], []
    max_match_count = 0
    total = len(df_a)
    ...
    if set_progress_status:
        set_progress_status("写入结果到新Excel")
    wb = Workbook()
    ws = wb.active
    ...
    if not vertical:
        ...
        # 竖向排列（每个基因一列，匹配结果向下扩展）
        for idx, gene in enumerate(df_a[gene_column_a]):
            ws.cell(row=1, column=idx + 1, value=gene)
            matches = all_matches[idx]
            flags = all_flags[idx]
            for j in range(max_match_count):
                val = matches[j] if j < len(matches) else ""
                cell = ws.cell(row=2 + j, column=idx + 1, value=val)
                if j < len(flags) and flags[j]:
                    cell.fill = blue_fill
                    col_letter = ws.cell(row=1, column=idx + 1).column_letter
                    highlight_cells.append(f"{col_letter}{2 + j}")
